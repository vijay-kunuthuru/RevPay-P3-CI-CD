...
public class AuthService {
    ...
    public void registerUser(SignupRequest request) {
        ...
        // TODO: Call Wallet Service to initialize wallet via Feign or Messaging
        log.info("TODO: Initialize wallet for UserID: {} in Wallet Service", savedUser.getUserId());
    ...
    public void verifyTransactionPin(Long userId, String pin) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new ResourceNotFoundException("User not found"));

        if (!passwordEncoder.matches(pin, user.getTransactionPinHash())) {
            log.warn("PIN_VERIFICATION_FAILED | Incorrect PIN for UserID: {}", userId);
