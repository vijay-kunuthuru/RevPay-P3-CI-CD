...
export class AdminUsersComponent implements OnInit {
    ...
    toggleUserStatus(user: any) {
        if (user.role === 'ADMIN') {
            this.notify.error("Cannot change status of ADMIN users.");
            return;
        }
        ...
        this.notify.confirm(`Are you sure you want to ${action} user ${user.email}?`).then((res: any) => {
            if (res.isConfirmed) {
                this.adminService.updateUserStatus(user.userId, newStatus).subscribe({
                    next: (res) => {
                        if (res.success) {
                            user.isActive = newStatus;
                            this.notify.success(`User successfully ${action}d.`);
                        } else {
                            this.notify.error('Failed to update status: ' + res.message);
                        }
                    },
                    error: (err) => {
                        this.notify.error('Error updating status: ' + (err.error?.message || 'Unknown error'));
                    }
                });
            }
        });
